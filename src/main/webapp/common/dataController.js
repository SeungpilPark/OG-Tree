...
DataController.prototype = {
    ...
    addFolderOutRelation: function (parentData, parentView, newItem, parentItem, parentType, parentId) {
        ...
        if (existRelItem.getItemCount() < 1) {
            try {
                ...
                toastr.error('Failed to create ' + relType + ' Relation : ' + parentId + ' to ' + newItemId);
            }
        }
    ...
    createActivity: function () {
        ...
        if (me.stdYN == 'Y' && me.thisItem.getProperty('state', '') != 'In Work') {
            toastr.error('This action is only available in "In Work" state.');
            return;
        } else if (me.stdYN == 'N' && me.thisItem.getProperty('state', '') != 'In Planning') {
            toastr.error('This action is only available in "In Planning" state.');
            return;
        }
        ...
        if (target.type == me.TYPE.ED) {
            ...
            if (existRelItem.getItemCount() == 0) {
                try {
                    ...
                    toastr.error('Failed to add ' + relType + ' Relation : ' + sourceId + ' to ' + targetId);
                }
            }
        } else {
            for (var i = 0, leni = selectedTargetList.length; i < leni; i++) {
                ...
                if (existRelItem.getItemCount() == 0) {
                    try {
                        ...
                        toastr.error('Failed to add ' + relType + ' Relation : ' + sourceId + ' to ' + selectargetId);
                    }
                } else {
